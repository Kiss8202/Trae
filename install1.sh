...
# ==================== 完整卸载 ====================
delete_self() {
    echo -e "${YELLOW}此操作将卸载 sing-box、删除所有节点配置、证书、快捷命令 sb 和当前脚本。${NC}"
    echo -e "${RED}警告：这将永久删除 sing-box 的所有数据！${NC}"
    echo ""
    echo -e "${CYAN}注意:${NC}"
    ...
    local keep_argo="N"
    if [[ -f /usr/bin/argo ]] || [[ -d /opt/argo ]]; then
        echo -e "${YELLOW}检测到 Argo Tunnel 已安装${NC}"
        read -p "是否保留 Argo Tunnel？(y/N): " keep_argo
        keep_argo=${keep_argo:-N}
        echo ""
    fi
    
    read -p "确认卸载？(y/N): " CONFIRM_DELETE
    CONFIRM_DELETE=${CONFIRM_DELETE:-N}
    ...
    # 仅在不保留 Argo 的情况下清理 Argo Tunnel
    if [[ ! "$keep_argo" =~ ^[Yy]$ ]]; then
    ...
    if [[ "$keep_argo" =~ ^[Yy]$ ]]; then
        print_success "已完成 sing-box 卸载和脚本清理，Argo Tunnel 已保留。"
    else
